...
class SimplePoolingLayer(nn.Module):
    def __init__(
        ...
        if point_cloud_range is None:
            point_cloud_range = [-5.12 * 3, -5.12 * 3, -5.12 * 3,
                                 5.12 * 3, 5.12 * 3, 5.12 * 3]
        ...
        #
        self.grid_size = [
            int(round((self.point_cloud_range[3] - self.point_cloud_range[0]) / self.voxel_size)),
            int(round((self.point_cloud_range[4] - self.point_cloud_range[1]) / self.voxel_size)),
            int(round((self.point_cloud_range[5] - self.point_cloud_range[2]) / self.voxel_size)),
        ]

        self.grid_min = [
            int(np.floor(self.point_cloud_range[0] / self.voxel_size)),
            int(np.floor(self.point_cloud_range[1] / self.voxel_size)),
            int(np.floor(self.point_cloud_range[2] / self.voxel_size)),
        ]
        self.grid_max = [
            self.grid_min[0] + self.grid_size[0] - 1,
            self.grid_min[1] + self.grid_size[1] - 1,
            self.grid_min[2] + self.grid_size[2] - 1,
        ]
        ...
        self.grid_conv = ME.MinkowskiConvolution(
            channels[0], channels[1], kernel_size=grid_kernel_size, dimension=3
        )
        self.grid_bn = ME.MinkowskiBatchNorm(channels[1])
        self.grid_relu = ME.MinkowskiELU()

        if self.pooling:
            self.pooling_conv = ME.MinkowskiConvolution(
                channels[1], channels[2], kernel_size=grid_num, dimension=3
            )
    ...
    def forward(self, sp_tensor, grid_points, grid_corners=None, box_centers=None, batch_size=None):
        ...
        grid_coords = grid_points.clone()
        grid_coords[:, 1:4] = torch.floor(grid_points[:, 1:4] / self.voxel_size)
        grid_coords = grid_coords.long()

        grid_coords[:, 1] = torch.clamp(
            grid_coords[:, 1],
            min=self.grid_min[0] + 1,
            max=self.grid_max[0] - 1
        )
        grid_coords[:, 2] = torch.clamp(
            grid_coords[:, 2],
            min=self.grid_min[1] + 1,
            max=self.grid_max[1] - 1
        )
        grid_coords[:, 3] = torch.clamp(
            grid_coords[:, 3],
            min=self.grid_min[2] + 1,
            max=self.grid_max[2] - 1
        )

        grid_coords_positive = grid_coords[:, 1:4].clone()
        grid_coords_positive[:, 0] -= self.grid_min[0]
        grid_coords_positive[:, 1] -= self.grid_min[1]
        grid_coords_positive[:, 2] -= self.grid_min[2]

        merge_coords = (
                grid_coords[:, 0] * self.scale_xyz
        ...
        unq_grid_coords[:, 1] += self.grid_min[0]
        unq_grid_coords[:, 2] += self.grid_min[1]
        unq_grid_coords[:, 3] += self.grid_min[2]

        # ROI head 的 voxel_size = backbone_voxel_size * coord_key
        ...
        if self.pooling:
            ...
            fake_grid_coords = fake_grid_coords.unsqueeze(0).repeat(grid_coords.shape[0] // fake_grid_coords.shape[0],
                                                                    1, 1)  # (batch*nums_rois, 7**3, 3)
            # fake center
            ...
            fake_grid_idx = fake_batch_idx.reshape([-1, 1, 1]).repeat(1, fake_grid_coords.shape[1],
                                                                      1)  # (batch*nums_rois, 7**3, 1)
            fake_grid_coords = torch.cat([fake_grid_idx, fake_grid_coords], dim=-1).reshape(
                [-1, 4]).int()  # (batch*nums_rois*7**3, 4)
...
class CAGroup3DRoIHead(nn.Module):
    def __init__(self, model_cfg, cls_loss_type='BinaryCrossEntropy', reg_loss_type='smooth-l1', **kwargs):
        ...
        for i in range(len(self.mlps)):  # different feature source, default only use semantic feature
            ...
            pool_layer = SimplePoolingLayer(channels=mlp, grid_kernel_size=roi_conv_kernel, grid_num=grid_size, \
                                            voxel_size=voxel_size * coord_key, coord_key=coord_key,
                                            point_cloud_range=point_cloud_range, pooling=self.use_center_pooling)
    ...
    def init_weights(self):
        ...
        layers_list = [self.shared_fc_layer, self.reg_fc_layers] if not self.use_center_pooling else [
            self.reg_fc_layers]
    ...
    def roi_grid_pool(self, input_dict):
        ...
        middle_feature_list = [input_dict['middle_feature_list'][i] for i in
                               self.middle_feature_source]  ## sptensor from backbone3D
    ...
    def forward_train(self, input_dict):
        pred_boxes_3d = input_dict[
            'pred_bbox_list']  # [list1,…, listB], list_i: (bboxes [K_i, 7], scores [K_i], labels [K_i])
    ...
    def reoder_rois_for_refining(self, pred_boxes_3d):
        ...
        # Keep yaw convention unchanged to stay consistent with GT / decoder
        if use_sem_score:
    ...
    def simple_test(self, input_dict):
        ...
        if input_dict.get('predict_roi_mode', True):
            ...
            pred_dict = dict(batch_box_preds=[], batch_score_preds=[], batch_cls_preds=[], batch_embedding=[],
                             batch_roi=[], batch_semantic_score_preds=[], \
                             batch_box_preds_shrink=[], batch_score_preds_shrink=[], batch_cls_preds_shrink=[],
                             batch_embedding_shrink=[], batch_roi_shrink=[], batch_semantic_score_preds_shrink=[])
    ...
    def get_boxes(self, input_dict, img_meta, thres=None):
        ...
        batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
            batch_size=batch_size, rois=roi_roi, cls_preds=rcnn_cls, box_preds=rcnn_reg, roi_labels=roi_labels,
            roi_sem_scores=roi_sem_scores
        )  # [B, num_rois, 7]
    ...
    def get_boxes_simple(self, input_dict, img_meta, thres=None):
        ...
        batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
            batch_size=batch_size, rois=roi_roi, cls_preds=None, box_preds=rcnn_reg, roi_labels=None,
            roi_sem_scores=None
        )  # [B, num_rois, 7]
    ...
    def _nms(self, bboxes, scores, labels, embeddings, rois, semantic_scores, img_meta, thres=None):
        ...
        nms_embeddings, nms_rois, nms_semantic_scores = [], [], []

        for i in range(n_classes):
            # TROUT / PCDet labels are 1-based
            cur_label = i + 1

            if scores.ndim == 2:
                ids = (labels == cur_label) & (scores[:, i] > self.test_score_thr) & (bboxes.sum(dim=1) != 0)
            else:
                ids = (labels == cur_label) & (scores > self.test_score_thr) & (bboxes.sum(dim=1) != 0)
            ...
            nms_labels.append(
                bboxes.new_full(class_scores[nms_ids].shape, cur_label, dtype=torch.long)
            )
        ...
        if len(nms_bboxes) > 0:
            ...
            nms_labels = bboxes.new_zeros((0,), dtype=torch.long)
            nms_embeddings = bboxes.new_zeros((0, embeddings.shape[-1]))
            nms_rois = bboxes.new_zeros((0, rois.shape[-1]))
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds, roi_labels=None, gt_bboxes_3d=None,
                                 gt_labels_3d=None, roi_sem_scores=None):
        """
        Args:
            batch_size:
            rois: (B, N, 7)
        ...
        batch_box_preds = self.box_coder.decode_torch(batch_box_preds, local_rois).view(-1,
                                                                                        code_size)  # [B, num_rois, 6/7]
            ...
            if self.use_iou_loss and fg_sum > 0:
                ...
                rcnn_boxes3d = self.box_coder.decode_torch(
                    fg_rcnn_reg.view(batch_anchors.shape[0], -1,
                                     code_size + 1 if self.encode_angle_by_sincos else code_size), batch_anchors
                ).view(-1, code_size)
