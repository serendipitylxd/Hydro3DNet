...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        """
        ## batch_rois, batch_gt_of_rois: (batch_size, roi_per_image, 7)
        ## batch_roi_labels, batch_gt_label_of_rois: (batch_size, roi_per_image)
        ## batch_roi_ious: IoU of roi and GT
        ## batch_roi_scores: cls_score[cls_i] * centerness for roi
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        rois = batch_dict['rois'] # (b, num_max_rois, 7)
        roi_scores = batch_dict['roi_scores']
        roi_labels = batch_dict['roi_labels']
        gt_boxes = batch_dict['gt_bboxes_3d'] # [list1, ..., listb]
        ...
        for index in range(batch_size):
            ...
            #
            cur_labels, cur_gt = gt_labels[index].clone(), gt_boxes[index].clone()
    ...
    def subsample_rois(self, max_overlaps):
        ...
        fg_rois_per_image = int(np.round(self.fg_ratio * self.roi_per_image)) # 0.9*128
    ...
    def get_max_iou_with_same_class(rois, roi_labels, gt_boxes, gt_labels):
        ...
        :return:
        """
        max_overlaps = rois.new_zeros(rois.shape[0]) # (M), iou
        gt_assignment = roi_labels.new_zeros(roi_labels.shape[0]) # (M), index of gt
        # each class
