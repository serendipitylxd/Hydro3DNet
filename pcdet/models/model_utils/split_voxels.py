...
def check_repeatv2(features, indices):
    ...
    return features_unique, indices_unique
...
def split_voxels_v2(x, b, voxel_importance, kernel_offsets, pruning_mode="topk", pruning_ratio=0.5):
    index = x.indices[:, 0]
    batch_index = index==b
    indices_ori = x.indices[batch_index]
    features_ori = x.features[batch_index]
    ...
    indices_with_imp = (indices_im_kernels + voxel_kerels_offset).view(-1, indices_im_kernels.shape[-1])
    ...
    if indices_im.sum()>0:
        features_im, coords_im = check_repeatv2(features_im, coords_im)
