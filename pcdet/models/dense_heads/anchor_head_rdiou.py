...
class AnchorHeadRDIoU(AnchorHeadTemplate):
    ...
    def get_rdiou(self, bboxes1, bboxes2):
        ...
        #bboxes1 = bboxes1.detach()
        #bboxes2 = bboxes2.detach()
    ...
    def get_clsreg_targets(self):
        ...
        # Enlarge positive samples for all anchors
        num_anchors_per_loc = box_cls_labels.shape[-1] // (h * w)  # Original shape might differ; adjust as needed
        box_cls_labels = box_cls_labels.view(batch_size, h, w, -1)

        expanded_labels = []
        for an_idx in range(box_cls_labels.shape[-1]):
            anchor_label = box_cls_labels[:, :, :, an_idx].clone().unsqueeze(-1)
            # Apply rolling for each anchor; adjust shifts based on your requirements
            # Example: Shift along height and width for each anchor
            shifted_h = anchor_label.roll(shifts=1, dims=1)
            shifted_w = anchor_label.roll(shifts=1, dims=2)
            expanded_an = torch.clamp(anchor_label + shifted_h + shifted_w, 0, 1)
            expanded_labels.append(expanded_an)

        box_cls_labels = torch.cat(expanded_labels, dim=-1)
        ...
        #print("rdiou stats:", rdiou.min().item(), rdiou.mean().item(), rdiou.max().item())

        # filter the background.
        with torch.no_grad():
            #print("box_cls_labels.shape:", box_cls_labels.shape, ".rdiou.detach().shape:", rdiou.detach().shape)
    ...
    def get_rdiou_guided_cls_loss(self):
        ...
        positives = box_cls_labels > 0
        #print(f"Positives per batch: {positives.sum().item()} / {positives.numel()}")
        ...
        cls_targets_one_hot = torch.zeros(
            (batch_size, cls_targets.shape[1], self.num_class),
            device=cls_targets.device
        )
        cls_targets_one_hot.scatter_(-1, cls_targets.unsqueeze(-1), 1.0)
        cls_preds = cls_preds.view(batch_size, -1, self.num_class)

        cls_loss_src = self.cls_loss_func(cls_preds, cls_targets_one_hot, weights=cls_weights)
    ...
    def forward(self, data_dict):
        ...
        c1 = box_preds[..., :3]
        c2 = box_preds[..., 3:6].clamp(max=3.0)
        c3 = box_preds[..., 6:]
        box_preds = torch.cat([c1, c2, c3], dim=-1)
        ...
        if not self.training or self.predict_boxes_when_training:
            batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
                batch_size=data_dict['batch_size'],
                cls_preds=cls_preds, box_preds=box_preds, dir_cls_preds=dir_cls_preds, data_dict=data_dict
            )
