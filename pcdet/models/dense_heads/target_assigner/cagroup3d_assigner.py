...
class CAGroup3DAssigner(BaseAssigner):
    ...
    def assign(self, points_list, gt_bboxes_ori, gt_labels_ori):
        ...
        for cls_id in range(class_num):
            ...
            select_inds = torch.nonzero((gt_labels_ori == cls_id)).squeeze(1) # selected box index
            if len(select_inds) == 0:
                ...
                n_boxes = len(select_inds) # number of selected classes boxes
                volumes = volume(gt_bboxes_ori).to(points.device)[select_inds] # (nboxes, )
                volumes = volumes.expand(n_points, n_boxes).contiguous()
                ...
                volumes = torch.where(inside_top_centerness_mask, volumes, torch.ones_like(volumes) * float_max) # (npoints, nboxes)
                min_area, min_area_inds = volumes.min(dim=1)

                labels = gt_labels[min_area_inds] # (npoints, ) to filter the satisfied points
                # labels = torch.where(min_area == float_max, -1, labels)
                labels = torch.where(min_area == float_max, -labels.new_ones(labels.shape), labels)
                bbox_targets = bbox_targets[range(n_points), min_area_inds] # (npoints, 7) for regression
                centerness_targets = compute_centerness(bbox_targets) # (npoints, )
                gt_bbox_targets = gt_bboxes[range(n_points), min_area_inds].clone() # (npoints, 7) corresponding target
    ...
    def assign_semantic(self, points, gt_bboxes, gt_labels, n_classes):
        ...
        volumes = volume(gt_bboxes).to(points.device) # (n_box,)
        volumes = volumes.expand(n_points, n_boxes).contiguous()
        inside_gt_bbox_mask = find_points_in_boxes(points, gt_bboxes, expanded_volumes=volumes) # (n_points, n_box)

        # if there are still more than one objects for a location,
        # we choose the one with minimal area
        volumes = torch.where(inside_gt_bbox_mask, volumes, torch.ones_like(volumes) * float_max)
        bk_mask = inside_gt_bbox_mask.sum(dim=1) != 0
        min_area, min_area_inds = volumes.min(dim=1) # (n_points,)
