...
class FSHNet_argo2(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        ...
        self.slotformer_stage0 = spconv.SparseSequential(
            SlotFormer(afd_dim, 4, num_layers=2, win_size=12, shift=True, cwi=True), 
            nn.BatchNorm1d(afd_dim, eps=1e-3, momentum=0.01),
        )

        self.slotformer_stage1 = spconv.SparseSequential(
            SlotFormer(afd_dim, 4, num_layers=2, win_size=12, shift=True, cwi=True), 
            nn.BatchNorm1d(afd_dim, eps=1e-3, momentum=0.01),
        )
    ...
    def forward(self, batch_dict):
        ...
        x = self.slotformer_stage0(x)

        x = self.slotformer_stage1(x)
