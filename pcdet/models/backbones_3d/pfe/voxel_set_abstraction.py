...
def sample_points_with_roi2(rois, points, r_add=2.0, h_add=1.0):
    """
    Proposal-centric cylindrical filtering.

    Args:
        rois:   (M, 7 + C), roi boxes in format [x, y, z, dx, dy, dz, heading, ...]
        points: (N, 3), xyz points
        r_add:  extra horizontal radius margin
        h_add:  extra vertical half-height margin
    ...
    """
    if points.shape[0] == 0:
        return points, points.new_zeros((0,), dtype=torch.bool)

    if rois.shape[0] == 0:
        point_mask = points.new_zeros((points.shape[0],), dtype=torch.bool)
        sampled_points = points[:1] if points.shape[0] > 0 else points
        return sampled_points, point_mask

    # point xy to roi center xy
    diff_xy = points[:, None, 0:2] - rois[None, :, 0:2]          # (N, M, 2)
    dist_xy = torch.norm(diff_xy, dim=-1)                        # (N, M)

    # use half diagonal in BEV as base radius, then enlarge by r_add
    roi_half_r = torch.norm(rois[:, 3:5] / 2.0, dim=-1)          # (M,)
    valid_xy = dist_xy <= (roi_half_r[None, :] + r_add)          # (N, M)

    # vertical filtering
    diff_z = torch.abs(points[:, None, 2] - rois[None, :, 2])    # (N, M)
    roi_half_h = rois[:, 5] / 2.0                                # (M,)
    valid_z = diff_z <= (roi_half_h[None, :] + h_add)            # (N, M)

    # keep points that satisfy any roi
    point_mask = (valid_xy & valid_z).any(dim=1)                 # (N,)
