...
def sample_points_with_high_level_area(high_level_area, roi_points, num_max_points_of_part=200000):
    """
    Filter ROI points that fall inside any predefined high-level 3D region
    (each region is an axis-aligned bounding box).

    Args:
        high_level_area: (M, 6) Tensor, boxes as [x_min, y_min, z_min, x_max, y_max, z_max]
        roi_points: (N, 3) Tensor, input point cloud (already ROI-filtered upstream)
        num_max_points_of_part: int, chunk size to process points in blocks to save memory

    Returns:
        sampled_points: (K, 3) Tensor, points located within at least one high-level region
        high_level_area_point_mask: (N,) BoolTensor, mask over roi_points indicating selection
    """
    # Handle empty input early
    if roi_points.shape[0] == 0:
        return roi_points, torch.zeros((0,), dtype=torch.bool)

    # Extract region bounds and move to the same device as points
    device = roi_points.device
    lower_bounds = high_level_area[:, :3].to(device)  # (M, 3)
    upper_bounds = high_level_area[:, 3:].to(device)  # (M, 3)

    if roi_points.shape[0] <= num_max_points_of_part:
        # Single-pass check for all points
        points_expanded = roi_points.unsqueeze(1)  # (N, 1, 3)
        # Per-dimension inclusion test against each region (broadcast to N x M x 3)
        in_bounds = (
            (points_expanded >= lower_bounds.unsqueeze(0)) &
            (points_expanded <= upper_bounds.unsqueeze(0))
        )
        # A point is selected if it is inside all three dims of ANY region
        point_mask = in_bounds.all(dim=-1).any(dim=1)
    else:
        # Block-wise processing to reduce peak memory usage
        point_mask_list = []
        start_idx = 0
        while start_idx < roi_points.shape[0]:
            end_idx = start_idx + num_max_points_of_part
            chunk = roi_points[start_idx:end_idx]
            chunk_expanded = chunk.unsqueeze(1)  # (chunk_size, 1, 3)

            in_bounds = (
                (chunk_expanded >= lower_bounds.unsqueeze(0)) &
                (chunk_expanded <= upper_bounds.unsqueeze(0))
            )
            # Inside all dims of any region
            chunk_mask = in_bounds.all(dim=-1).any(dim=1)
            point_mask_list.append(chunk_mask)
            start_idx = end_idx

        point_mask = torch.cat(point_mask_list, dim=0)

    # Guarantee at least one point is returned to avoid empty tensors downstream
    if point_mask.sum() == 0:
        sampled_points = roi_points[:1]
    else:
        sampled_points = roi_points[point_mask]
...
class HierarchicalVoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        #  HIGH_LEVEL_AREA: [[40.0, 0.0, -5.12, 62.40, 102.4, 4.0], [40.0, 0.0, 4.0, 62.40, 102.4, 5.12]]
        self.high_level_area = torch.tensor(self.model_cfg.HIGH_LEVEL_AREA)
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        #print("  roi_boxes.shape: ", roi_boxes.shape)
        #print("  sampled_points.shapebf: ",sampled_points.shape)
        ...
        #print("  sampled_points.shapeaf: ",sampled_points.shape)
        return sampled_points

def sectorized_proposal_centric_sampling_hierarchical(self, roi_boxes, points):
    """
    Hierarchical Sectorized Proposal-Centric (SPC) Sampling.

    Args:
        roi_boxes: (M, 7 + C) Tensor, ROI boxes [x, y, z, dx, dy, dz, heading, ...]
        points: (N, 3) Tensor, candidate point cloud

    Returns:
        sampled_points: (K, 3) Tensor, final sampled keypoints
                        K = self.model_cfg.NUM_KEYPOINTS
    """
    # Stage 1: ROI-aware pre-sampling
    # Keep points that are close to any ROI (within box half-diagonal + margin)
    sampled_points, roi_mask = sample_points_with_roi(
        rois=roi_boxes, points=points,
        sample_radius_with_roi=self.model_cfg.SPC_SAMPLING.SAMPLE_RADIUS_WITH_ROI,
        num_max_points_of_part=self.model_cfg.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000)
    )

    # Stage 2: High-level region filtering
    # Further filter sampled points by whether they fall inside predefined "high-level" regions
    high_level_points, high_level_mask = sample_points_with_high_level_area(
        self.high_level_area,   # Predefined regions stored in the class
        sampled_points,
        self.model_cfg.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000)
    )

    # Stage 3: Hierarchical sectorized FPS sampling
    num_target = self.model_cfg.NUM_KEYPOINTS
    num_sectors = self.model_cfg.SPC_SAMPLING.NUM_SECTORS
    high_level_sampling_ratio = self.model_cfg.HIGH_LEVEL_SAMPLING_RATIO

    # Priority sampling from high-level regions
    if high_level_points.shape[0] > 0:
        high_sampled = sector_fps(
            points=high_level_points,
            num_sampled_points=min(num_target * high_level_sampling_ratio, high_level_points.shape[0]),
            num_sectors=num_sectors
        )
        sampled_count = high_sampled.shape[0]
    else:
        # No points in high-level regions → start with empty tensor
        high_sampled = torch.zeros((0, 3), device=points.device)
        sampled_count = 0

    # If high-level points are not enough, complement with other regions
    if sampled_count < num_target:
        # Get points outside high-level areas
        complement_mask = ~high_level_mask
        complement_points = sampled_points[complement_mask]

        # Remaining number of keypoints to be filled
        remain = num_target - sampled_count

        if complement_points.shape[0] > 0:
            # Run the same sectorized FPS on complementary points
            complement_sampled = sector_fps(
                points=complement_points,
                num_sampled_points=min(remain, complement_points.shape[0]),
                num_sectors=num_sectors
            )
            # Merge high-level samples and complementary samples
            high_sampled = torch.cat([high_sampled, complement_sampled], dim=0)

    # Ensure the final number of sampled points matches exactly num_target
    return high_sampled[:num_target]
    ...
    def get_sampled_points(self, batch_dict):
        ...
        for bs_idx in range(batch_size):
            ...
            if self.model_cfg.SAMPLE_METHOD == 'FPS':
                ...
                #)
                cur_keypoints = self.sectorized_proposal_centric_sampling_hierarchical(
                    roi_boxes=batch_dict['rois'][bs_idx], points=sampled_points[0]
                )

                bs_idxs = cur_keypoints.new_ones(cur_keypoints.shape[0]) * bs_idx
                #print("  sampled_points.shape[1]: ",sampled_points.shape[1])
    ...
    def forward(self, batch_dict):
        ...
        #print("high_level_area.shape:",self.high_level_area.shape,"batch_dict['points'].shape",batch_dict['points'].shape)
        point_features_list = []
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
