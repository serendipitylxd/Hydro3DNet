...
class BiResNet(nn.Module):
    ...
    def __init__(self,
        ...
        ## low resolution feature leraning, downsample+increase dim
        ## stride=2
        ...
        ## decrease dim (combine with interpolation, to match dimension for fusion in high resolution)
        ...
        ## downsample+increase dim (to match dimension for fusion in low resolution)
    ...
    def forward(self, input_dict):
        ...
        x = self.layer1(x) # 2, 1p
        layers.append(x)

        x = self.layer2(self.relu(x)) # 4, 2p
        layers.append(x)
  
        x = self.layer3(self.relu(x)) # 8, 4p
        layers.append(x)
        x_ = self.layer3_(self.relu(layers[1])) # 4, hp=2p

        ## fusion in low resolution
        x = x + self.down3(self.relu(x_)) # 8, 4p
        ## fusion in high resolution
        x_f = x_.F + self.compression3(self.relu(layers[2])).features_at_coordinates(x_.C.float()) # 4, hp=2p
        x_ = ME.SparseTensor(features=x_f,
            coordinate_manager=x_.coordinate_manager, coordinate_map_key=x_.coordinate_map_key) # 4, hp=2p

        # TODO: test
        if self.augment:
            temp = x_

        x = self.layer4(self.relu(x)) # 16, 8p
        layers.append(x)
        x_ = self.layer4_(self.relu(x_)) # 4, hp=2p

        ## fusion in low resolution
        x = x + self.down4(self.relu(x_)) # 16, 8p
        ## fusion in high resolution
        x_f = x_.F + self.compression4(self.relu(layers[3])).features_at_coordinates(x_.C.float()) # 4, hp=2p
        x_ = ME.SparseTensor(features=x_f,
            coordinate_manager=x_.coordinate_manager, coordinate_map_key=x_.coordinate_map_key) # 4, hp=2p

        x = self.layer5(self.relu(x)) # 32, 16p
        x_ = self.layer5_(self.relu(x_)) # 4, 2hp=4p
        ## fusion in high resolution
        x_f = x_.F + self.spp(x).features_at_coordinates(x_.C.float()) # 4, 2hp=4p
        x_ = ME.SparseTensor(features=x_f,
