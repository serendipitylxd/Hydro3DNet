...
class CustomDataset(DatasetTemplate):
    ...
    def get_label(self, idx):
        if self.split == 'test':  # Check if it is in test mode
            label_file = self.root_path / 'labels_test' / ('%s.txt' % idx)  # Use the point_test folder
        else:
    ...
    def get_lidar(self, idx):
        if self.split == 'test':  # Check if it is in test mode
            lidar_file = self.root_path / 'points_test' / ('%s.npy' % idx)  # Use the point_test folder
        else:
            lidar_file = self.root_path / 'points' / ('%s.npy' % idx)  # Other modes use the points folder
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    test_filename = save_path / ('custom_infos_%s.pkl' % test_split)  # Add the file name of test
    ...
    custom_infos_test = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features  # test sets are usually unlabeled, but this dataset does
    )
    ...
    # Create a groundtruth database only for the train set
    dataset.set_split(train_split)
