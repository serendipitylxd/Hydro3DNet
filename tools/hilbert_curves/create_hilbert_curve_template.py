...
#    return bit_data
def get_bit_data(point_locs: torch.Tensor, bit: int, chunk_size: int = 100_000) -> torch.Tensor:
    """
    Block calculation (point_locs >> bit) & 1 to avoid applying for excessive video memory at one time.
    chunk_size: The number of lines processed each time, which can be adjusted to be smaller or larger according to the GPU video memory as appropriate.
    """
    ...
    for start in range(0, N, chunk_size):
        ...
        chunk = point_locs[start:end]            # [chunk_size, num_dims]
        chunk_bits = ((chunk >> bit) & 1).to(device)
